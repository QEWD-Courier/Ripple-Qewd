...
for (heading in headings) {
  ...
  if (fs.existsSync(aqlFile)) {
    text = '';
    fs.readFileSync(aqlFile).toString().split(/\r?\n/).forEach(function(line){
      text = text + line;
    })
...
for (heading in headings) {
  ...
  if (fs.existsSync(sqlFile)) {
    text = '';
    fs.readFileSync(sqlFile).toString().split(/\r?\n/).forEach(function(line){
      text = text + line;
    })
    if (!headings[heading].query) headings[heading].query = {}; 
    headings[heading].query.sql = text;
    console.log('*** ' + heading + ': ' + headings[heading].query.sql);
  }
}
