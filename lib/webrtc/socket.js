...
io.on('connection', function (socket) {
    ...
    socket.on('user:init', (user) => {
        ...
        if (usersList[user.username].isPatient()) {
            console.log('[user:init]', 'it is patient');
    ...
    socket.on('appointment:init', (data) => {
        ...
        if (patient) {
            console.log('[appointment:init] patient is already in site');
            ...
            console.log('[appointment:init] patient is not in site yet');
    ...
    socket.on('disconnect', () => {
        ...
        if (socketsList[socket.id].user) {
            ...
            if (!user.sid.length) {
                delete patientsList[user.username];
                delete patientsWaitersList[user.username];
