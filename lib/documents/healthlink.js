...
function getInternalPatientId(document) {
  ...
  parser.parseString(document, function(err, data) {
    
   if(data !== undefined) {  
     patientId = data['REF_I12']['PID'][0]['PID.3'][0]['CX.1'][0];
   }
  });
...
function getTemplateId(documentType) {
  var templateMap = {
...
// TODO - remane result to discharge or referral
// TODO - cardinality
// Ask Rob about the finnished type - it's passed in to a lot of places. Comes frm QEWD? Used to report status eg errors?
function getDischarge(document, callback) { 
  var result = {};
  ...
  parser.parseString(document, function(err, data) {
   ...
   if(data !== undefined) {  
      ...
      result['ctx/time'] = data['REF_I12']['MSH']['MSH.7']["TS.1"];
      result['ctx/composer_name'] = data['REF_I12']['MSH']["MSH.6"]["HD.1"];
      result['ctx/composer_id'] = data['REF_I12']['MSH']["MSH.6"]["HD.2"];
      result['ctx/health_care_facility|name'] = data['REF_I12']['MSH']['MSH.4']["HD.1"]; 
      result['ctx/health_care_facility|id'] = data['REF_I12']['MSH']['MSH.4']["HD.2"];  
      
      result['discharge_summary/composer|id_scheme'] = data['REF_I12']['MSH']["MSH.6"]["HD.3"]; 
      result['discharge_summary/clinical_summary/clinical_synopsis/synopsis'] = data['REF_I12']['NTE']['NTE.3']['_'].trim();

      addDischargeIdentifier(data['REF_I12']['PID']['PID.3'], result);
    
      addDiagnoses(data['REF_I12']['DG1'], result);

      addProfessionalIdentifier(data['REF_I12']['REF_I12.PATIENT_VISIT']['PV1']['PV1.7'], result);
   }
  });
...
function getReferral(document, callback) {
  var result = {};
  ...
  parser.parseString(document, function(err, data) {    
    ...
    if(data !== undefined) {
      ...
      result['ctx/id_scheme'] = "iEHR";            
      result['ctx/time'] = data['REF_I12']['MSH']['MSH.7']["TS.1"];
      result['ctx/composer_name'] = data['REF_I12']['MSH']["MSH.4"]["HD.1"];
      result['ctx/health_care_facility|id'] = data['REF_I12']['MSH']['MSH.4']["HD.2"];
      
      result['referral/referral_details/service_request/referred_to_provider/identifier'] = data['REF_I12']['MSH']['MSH.6']["HD.1"];
      result['referral/referral_details/service_request/referral_control_number'] = data['REF_I12']['MSH']['MSH.10'];
      result['referral/referral_details/service_request/request:0/timing'] = "Boilerplate timing string";

      addReferralStatus(data['REF_I12']['RF1']['RF1.1']['CE.1'], result);

      result['referral/referral_details/referral_status/referral_type'] = data['REF_I12']['RF1']['RF1.3']['CE.1'];
      result['referral/referral_details/service_request/request:0/referral_type'] = data['REF_I12']['RF1']['RF1.3']['CE.1'];
      result['referral/referral_details/service_request/narrative'] = data['REF_I12']['RF1']['RF1.3']['CE.1'];

      addReferralPriority(data['REF_I12']['RF1']['RF1.3']['CE.2'], result);

      result['referral/referral_details/service_request/referring_provider/identifier']  = data['REF_I12']['RF1']['RF1.6']['EI.1'];
      result['referral/referral_details/referral_status/time']  = data['REF_I12']['RF1']['RF1.7']['TS.1'];
      result['ctx/health_care_facility|name']  = data['REF_I12']['REF_I12.PROVIDER_CONTACT'][0]['PRD']['PRD.4']['PL.1'];
      result['referral/referral_details/service_request/distribution:0/individual_recipient:0/gp/name_of_organisation']  = data['REF_I12']['REF_I12.PROVIDER_CONTACT'][0]['PRD']['PRD.4']['PL.1'];
      result['referral/referral_details/service_request/referring_provider/name_of_organisation']  = data['REF_I12']['REF_I12.PROVIDER_CONTACT'][0]['PRD']['PRD.4']['PL.1'];
      ...
      result['referral/referral_details/service_request/referred_to_provider/name_of_organisation']  = data['REF_I12']['REF_I12.PROVIDER_CONTACT'][0]['PRD']['PRD.3']['XAD.2'];

      addContactDetails(data['REF_I12']['REF_I12.PROVIDER_CONTACT'], result);

      addObservations(data['REF_I12']['REF_I12.OBSERVATION'], result);
    }
  });
...
function addObservations(observations, results) {
  for(observation in observations) {
    ...
    var resultNotes = observations[observation]['REF_I12.RESULTS_NOTES'];
    
    // result notes are optional
    if(resultNotes !== undefined) {
      if(Array.isArray(resultNotes)) {
        for(resultNote in resultNotes) {      
            var result = resultNotes[resultNote]
            var resultType = result['OBX']['OBX.3']['CE.2'].toLowerCase();

            // keep track of the result types
            resultTypeCount[resultType] = (resultTypeCount[resultType]+1) || 1 ;

            addResult(resultType, result, resultTypeCount[resultType], results);
        }
      }
      else {
        var resultType = resultNotes['OBX']['OBX.3']['CE.2'].toLowerCase();
...
function addContactDetails(contactDetails, result) {
  ...
  for(var contactDetail in contactDetails) {
      var xtn2 = contactDetails[contactDetail]['PRD']['PRD.5']['XTN.2']
      if(xtn2 !== undefined) {
        var contactDetailType = contactDetailsTypeMap[xtn2];
