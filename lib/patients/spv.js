...
module.exports = {
  ...
  getPatientSummary: function(args, callback) {
    ...
    if (patientId === 'null' || patientId === 'undefined' || typeof patientId === 'undefined') {
      callback({error: 'A null or undefined patientId was specified in the request'});
      return;
    }

    if (patientId === '') {
      callback({error: 'No patientId was specified in the request'});
      return;
    }

    // strip out any spaces from patient number

    patientId = patientId.split(' ').join('');

    if (!isNormalInteger(patientId)) {
      callback({error: 'Invalid patientId: ' + args.patientId});
      return;
    }
    console.log('*** getPatientSummary - args.patientId: ' + args.patientId + '; patientId: ' + patientId);

    patientSummary.get.call(this, patientId, args.session, callback);
  },
