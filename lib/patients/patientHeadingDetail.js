...
function patientHeadingDetail(args, finished) {

  var patientId = args.patientId;
  var headingName = args.heading;
  ...
  if (!headings[headingName]) {
    ...
    return finished({error: headingName + ' has not yet been added to middle-tier processing'});
  }
  ...
  if (!indexSource.exists) {
    return finished({error: 'Invalid sourceId ' + sourceId + ' for patient ' + patientId + ' / heading ' + heading});
  }
  var index = indexSource.getDocument();

  if (typeof index.recNo === 'undefined' || index.recNo === '') {
    return finished({error: 'Unable to use the index to sourceId ' + sourceId + ' for patient ' + patientId + ' / heading ' + heading + ': recNo is missing'});
  }
